...
class Trad
{
    ...
    {
        $surcharge = Jin::getAppPath() . Jin::getSurchargeRelativePath() . '/' . Jin::getRelativePathLanguage() . self::$langueCode . '/' . $fileName;

        if (Jin::getConfigValue('surcharge') && file_exists($surcharge)) {
            $data = parse_ini_file($surcharge);
        } else {
            $data = parse_ini_file(Jin::getJinPath() . Jin::getRelativePathLanguage() . self::$langueCode . '/' . $fileName);
        }
