...
namespace Diatem\Jin\Output\Components\Ui\Table;
...
use Diatem\Jin\Output\Components\Ui\Table\TableModel;
...
class Table extends UIComponent implements ComponentInterface {
    ...
    /**
     * Définit les en-tête de colonne. Si un tableau associatif est transmis
     * array('nom_col_technique' => 'Libelle colonne') il permet également de
     * définir les colonnes que l'on souhaite afficher ainsi que l'ordre.
     ...
     * @param \Diatem\Jin\Output\Components\Ui\Table\TableModel $tm    TableModel Appliquable
     */
     ...
     * @param \Diatem\Jin\Query\QueryResult $datasource
     */
     ...
     * @return \Diatem\Jin\Query\QueryResult
     */
    ...
    private function renderHeaders($html) {
        ...
        if ($this->headers) {
            ...
            if ($hc != ArrayTools::length($this->headers) && !ArrayTools::isAssociativeArray($this->headers)) {
    ...
    private function renderLine($line, $lineNum) {
        ...
        if($this->headers && ArrayTools::isAssociativeArray($this->headers)){
            $heads = ArrayTools::getAllKeys($this->headers);
        }
